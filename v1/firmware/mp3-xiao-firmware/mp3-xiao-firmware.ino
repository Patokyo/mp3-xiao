...
const unsigned char bitmap_play [] PROGMEM = {
	0x00, 0x00, 0x00, 0x00, 0x30, 0x00, 0x3c, 0x00, 0x3f, 0x00, 0x3f, 0xc0, 0x3f, 0xf0, 0x3f, 0xfc, 
	0x3f, 0xfc, 0x3f, 0xf0, 0x3f, 0xc0, 0x3f, 0x00, 0x3c, 0x00, 0x30, 0x00, 0x00, 0x00, 0x00, 0x00
};
...
const unsigned char bitmap_skip [] PROGMEM = {
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x04, 0x18, 0x04, 0x1e, 0x04, 0x1f, 0x84, 0x1f, 0xe4, 
	0x1f, 0xe4, 0x1f, 0x84, 0x1e, 0x04, 0x18, 0x04, 0x00, 0x04, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00
};
...
void loop() {
  ...
  if(digitalRead(SINGLE_PIN) == LOW && state.mode == "play"){
    ...
    // Ensure button released before checking for double press
    while (digitalRead(SINGLE_PIN) == LOW);  
    delay(50);  // debounce after release

    unsigned long doublePressStart = millis();
    while (millis() - doublePressStart < 400) {  // 400ms window
      if (digitalRead(SINGLE_PIN) == LOW) {
        doublePressed = true;
        while (digitalRead(SINGLE_PIN) == LOW);  // wait for release
        break;
      }
    }
